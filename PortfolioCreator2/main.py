...
class RisingSectorFundamentalUniverse(QCAlgorithm):
    ...
    def fine_selection_function(self, fine):
        ...
        # Get S&P 500 stocks (8 stocks)
        sp500_stocks = []
        if hasattr(self, 'sp500_tracker') and self.sp500_tracker is not None:
            try:
                sp500_stocks = self.sp500_tracker.get_top_missing_sp500_stocks(sector_universe, top_n=8)
                if sp500_stocks:
                    sp500_names = [s.value for s in sp500_stocks]
                    self.log(f"S&P 500 stocks (8 stocks): {sp500_names}")
                else:
                    self.log("No S&P 500 stocks available")
            except Exception as e:
