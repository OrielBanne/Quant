...
class IntegratedSP500Tracker:
    ...
    def process_fine_data_for_sp500(self, fine_data_list):
        ...
        try:
            ...
            if len(self.sp500_candidates) > 0:
                self.algorithm.log(f"S&P 500: {len(self.sp500_candidates)} candidates identified")
            else:
    ...
    def analyze_sp500_influence(self, current_universe_symbols, period_days=30):
        ...
        try:
            self.algorithm.log(f"S&P 500 Influence Analysis: Starting with {len(self.sp500_candidates)} candidates")
            
            if len(self.sp500_candidates) == 0:
                self.algorithm.log("No S&P 500 candidates available for analysis")
                return None
            
            # Calculate SPY performance
            self.algorithm.log(f"Calculating SPY performance for {period_days} days")
            spy_performance = self.calculate_spy_performance(period_days)
            if spy_performance is None:
                self.algorithm.log("SPY performance calculation failed")
                return None
            
            self.algorithm.log(f"SPY performance: {spy_performance['performance']:.2f}% over {period_days} days")
            
            # Calculate influence for each S&P 500 candidate
            influences = []
            successful_calculations = 0
            
            self.algorithm.log(f"Calculating influence for {len(self.sp500_candidates)} S&P 500 candidates")
            ...
            for symbol_str, data in self.sp500_candidates.items():
                ...
                if influence is not None:
                    ...
                    # Log some key stocks
                    if symbol_str in ["NVDA", "AAPL", "MSFT", "GOOGL", "TSLA"]:
                        self.algorithm.log(f"  {symbol_str}: {influence['performance']:+.1f}% performance, {influence['sp500_contribution']:+.3f}% S&P contribution")
            
            self.algorithm.log(f"Successfully calculated influence for {successful_calculations}/{len(self.sp500_candidates)} candidates")
            ...
            self.algorithm.log(f"Current universe symbols: {list(current_symbol_strings)}")
            ...
            for stock in positive_influences:
                ...
                if stock_symbol not in current_symbol_strings:
                    ...
                    self.algorithm.log(f"Missing: {stock_symbol} (not in current universe)")
                else:
                    self.algorithm.log(f"Found: {stock_symbol} (already in universe)")
            
            self.algorithm.log(f"S&P 500 analysis: {len(positive_influences)} positive contributors, {len(missing_influencers)} missing from universe")
            
            if len(positive_influences) > 0:
                self.algorithm.log("Top positive contributors:")
                for i, stock in enumerate(positive_influences[:5]):
                    self.algorithm.log(f"  {i+1}. {stock['symbol']} ({stock['sector']}): {stock['performance']:+.1f}% (S&P Impact: {stock['sp500_contribution']:+.3f}%)")
            
            if len(missing_influencers) > 0:
                self.algorithm.log("Missing influential stocks:")
                for i, stock in enumerate(missing_influencers[:5]):
                    self.algorithm.log(f"  {i+1}. {stock['symbol']} ({stock['sector']}): {stock['performance']:+.1f}% (S&P Impact: {stock['sp500_contribution']:+.3f}%)")
    ...
    def get_top_missing_sp500_stocks(self, current_universe_symbols, top_n=5):
        """Get top N missing S&P 500 influential stocks"""
        try:
            self.algorithm.log(f"Getting top {top_n} missing S&P 500 stocks from current universe of {len(current_universe_symbols)} stocks")
            
            analysis = self.analyze_sp500_influence(current_universe_symbols)
            
            if analysis and analysis['missing_influencers']:
                top_missing = analysis['missing_influencers'][:top_n]
                self.algorithm.log(f"Found {len(top_missing)} missing influential stocks to add")
                ...
                for stock in top_missing:
                    try:
                        ...
                        self.algorithm.log(f"Successfully created symbol for {stock['symbol']}")
                    except Exception as e:
                        self.algorithm.log(f"Error creating symbol for {stock['symbol']}: {str(e)}")
                        continue
                
                self.algorithm.log(f"Top {len(missing_symbols)} missing S&P 500 influential stocks identified")
                
                for i, stock in enumerate(top_missing[:len(missing_symbols)]):
                    self.algorithm.log(f"  {i+1}. {stock['symbol']} ({stock['sector']}): "
                                      f"{stock['performance']:+.1f}% (S&P Impact: {stock['sp500_contribution']:+.3f}%)")
                
                return missing_symbols
            else:
                self.algorithm.log("No missing influential stocks found in analysis")
            
            return []
            
        except Exception as e:
            self.algorithm.log(f"Error getting missing S&P 500 stocks: {str(e)}")
            return []
