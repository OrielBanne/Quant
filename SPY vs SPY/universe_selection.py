    ...
    def fine_selection_function(self, fine):
        ...
        try:
            ...
            for sector in self.selected_sectors:
                ...
                # Sort by score and take top 1 stock per sector (concentration)
                filtered_stocks.sort(key=lambda x: x[2], reverse=True)
                sector_filtered_stocks[sector] = filtered_stocks[:1]
            
            # Build sector-based universe (4 stocks: 1 per sector)
            sector_universe = build_final_universe(self.algorithm, sector_filtered_stocks, 4)
            ...
            self.algorithm.log(f"Final universe: {len(final_universe)} concentrated stocks")
