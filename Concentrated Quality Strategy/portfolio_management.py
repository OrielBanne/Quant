...
class PortfolioManager:
    ...
    def log_portfolio_status(self):
        ...
        if summary:
            # Get universe information with scores if available
            universe_info = ""
            if hasattr(self.algorithm, 'universe_symbols') and self.algorithm.universe_symbols:
                universe_symbols = [s.value for s in self.algorithm.universe_symbols if s != self.algorithm.spy]
                if universe_symbols:
                    # Try to get stock scores from the last universe selection
                    if hasattr(self.algorithm, 'last_stock_scores'):
                        score_parts = []
                        for symbol in universe_symbols:
                            if symbol in self.algorithm.last_stock_scores:
                                score_parts.append(f"{symbol}: {self.algorithm.last_stock_scores[symbol]:.1f}")
                            else:
                                score_parts.append(symbol)
                        universe_info = f"  Universe: {' '.join(score_parts)}"
                    else:
                        universe_info = f"  Universe: {', '.join(universe_symbols)}"
            
            self.algorithm.log(f"Portfolio: ${summary['total_value']:,.2f} "
                             f"({summary['position_count']} positions, "
                             f"{summary['cash_percentage']:.1%} cash){universe_info}")
