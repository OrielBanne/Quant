...
from charting_utils import ChartingManager
...
class RisingSectorFundamentalUniverse(QCAlgorithm):
    def initialize(self):
        ...
        self.charting_manager = ChartingManager(self)
        ...
        # Schedule S&P 500 analysis (will be called after universe is set up)
        self.schedule.on(self.date_rules.every_day(), 
                    self.time_rules.after_market_open(self.spy, 45), 
                    self.initialize_sp500_tracker)
        
        # Schedule daily chart updates
        self.schedule.on(self.date_rules.every_day(), 
                    self.time_rules.after_market_close(self.spy, 1), 
                    self.charting_manager.update_equity_tracking)
        
        # Schedule monthly performance summary
        self.schedule.on(self.date_rules.month_start(), 
                    self.time_rules.after_market_open(self.spy, 90), 
                    self.charting_manager.log_performance_summary)
    ...
    def initialize_sp500_tracker(self):
        """Initialize S&P 500 tracker after universe is set up"""
        try:
            if hasattr(self.universe_selector, 'sp500_tracker') and self.universe_selector.sp500_tracker is not None:
                if not hasattr(self.universe_selector.sp500_tracker, 'initialized'):
                    self.universe_selector.sp500_tracker.initialize()
                    self.universe_selector.sp500_tracker.initialized = True
        except Exception as e:
            self.log(f"Error initializing S&P 500 tracker: {str(e)}")
    ...
    def OnSecuritiesChanged(self, changes):
        ...
        try:
            ...
            # Reset holding start times for removed securities
            if hasattr(self, 'stock_holding_start'):
                for removed_security in changes.removed_securities:
                    if removed_security.symbol in self.stock_holding_start:
                        del self.stock_holding_start[removed_security.symbol]
            
            # Log universe changes (only if significant)
            if changes.added_securities and len(changes.added_securities) > 2:
