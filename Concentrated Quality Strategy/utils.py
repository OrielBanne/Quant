...
DEFAULT_SECTOR_FILTERS = {
    "Information Technology": {
        "pe_ratio_max": 150.0,  # Allow higher P/E for high-growth tech
        "pe_ratio_min": 5.0,
        "pb_ratio_max": 50.0,
        "roe_min": 0.08,  # Lower ROE threshold to catch growth stocks
        "roe_max": 0.80,  # Higher max for exceptional performers
        "revenue_growth_min": 0.10,  # Higher growth requirement
        "debt_to_equity_max": 1.5,
        "min_quarterly_revenue": 200_000_000  # Lower revenue threshold for growth stocks
    },
...
def calculate_fundamental_score(stock_data, sector):
    ...
    try:
        ...
        # Sector-specific weighting (growth-focused)
        if sector == "Information Technology":
            # Tech: Focus heavily on growth and momentum
            final_score = (
                growth_score * 0.45 +   # Growth: 45% (increased from 25%)
                roe_score * 0.25 +      # Profitability: 25% (reduced from 35%)
                pe_score * 0.15 +       # Valuation: 15% (reduced from 20%)
    ...
    for sector, stocks in sector_filtered_stocks.items():
        if stocks:
            for stock_ticker, fine_data, score in stocks:
                all_stocks_with_scores.append((stock_ticker, fine_data, score))
